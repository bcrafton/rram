...
class rram:
    ...
    def step(self, Vin, dt):
        ...
        I1 = I0 * np.pi*(WCF * WCF/ 4 - w * w / 4)*np.exp(-L0 / XT) * np.sinh(Vtb / VT)
        RCF = rou * (L0-x)/(np.pi * w * w/ 4)         
        Vg = Vtb - (Itb - I1) * RCF
        ...
        if ( Vtb > 0 ):
            ...
            if ( x > 0 ):
                dxs = -a*f*np.exp(-(Ea-Vg*alpha*Z/x)/(Kb*Temp))
                if ( abs(dxs) <= 1e2 ):
                    dx = dxs
                else:
                    dx = -1e2
            ...
            dw = 0 ;
            if ( x <= 0 ): 
                dxr1 = a*f*np.exp(-(Ei+r*Z*Vg)/(Kb*Temp))
                dx = dxr1
            elif ( x > 0 ):
                if ( x < L0 ):
	                  dxr1 = a * f * np.exp(-(Ei + r * Z * Vg) / (Kb * Temp))
	                  dxr2 = a * f * np.exp(-Eh / (Kb * Temp))*sinh(alphah * Z * -1 * Vg/ (x * Kb * Temp))
        ...
        if ( Vtb == 0 ):
            ...
            if ( x > 0 ):
                Itb = I1 + I0 * np.pi * (w * w / 4) * np.exp(-x / XT) * np.sinh(Vg/VT)
            elif ( x <= 0 ):
                Itb = I1 + Vtb / (rou * L0 / (np.pi * w * w / 4))  
        ...
        if ( switch ):
            if ( Vtb > 0 ):
                if ( dw == 0 ):
	                  ...
	                  cf_random_ddt = np.random.uniform(low=0., high=1.) * deltaCF
                ...
                if ( x < crit_x ):
	                  ...
	                  gap_random_ddt = np.random.uniform(low=0., high=1.) * deltaGap
        ...
        self.x += (dx + gap_random_ddt) * dt
        self.w += (dw + cf_random_ddt) * dt
