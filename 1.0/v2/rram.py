...
class rram:
    ...
    def step(self, Vin, dt):
        ...
        self.T_cur = self.T_ini + np.abs( self.Vtb * self.Itb * self.Rth)
        ...
        if (self.gamma * np.abs( self.Vtb ) / self.tox) < self.F_min:
            self.gamma = 0.
        ...
        # self.gap_random_ddt = 0.
        self.gap_random_ddt = np.random.normal(loc=0., scale=1.) * self.deltaGap / (1. + np.exp((self.T_crit - self.T_cur)/self.T_smth));
