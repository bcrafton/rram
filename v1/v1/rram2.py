...
def step(self, Vin, dt):
    ...
    # 			rand_seed = rand_seed_ini;
		...
		if ((self.gamma * abs(Vtb) / self.tox) < self.F_min):
        self.gamma = 0
    ...
    # gap time derivative - variation part
		...
		self.gap_random_ddt = 0.5 * self.deltaGap / (1. + np.exp((self.T_crit - self.T_cur) / self.T_smth))
    ...
    # look this up in manual we downloaded.
		...
		self.gap += (gap_ddt + gap_random_ddt) * dt
