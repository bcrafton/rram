...
# 			rand_seed = rand_seed_ini;
		...
		gamma = gamma_ini - beta * np.power(((gap)/1e-9), 3)
    ...
    gap_ddt = -Vel0 * np.exp(-q * Ea / kb / T_cur) * np.sinh(gamma * a0 / tox * q * Vtb / kb / T_cur)
    ...
    # gap time derivative - variation part
		...
		gap_random_ddt = 0.5 * deltaGap / (1. + np.exp((T_crit - T_cur)/T_smth))
		...
		Itb = I0 * np.exp(-gap / g0) * sinh(Vtb / V0)
